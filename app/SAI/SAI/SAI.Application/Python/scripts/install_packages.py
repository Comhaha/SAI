...
사용법:
    # 패키지 설치 기본 함수
    from scripts.install_packages import install_packages_with_progress
    result = install_packages_with_progress(["ultralytics", "opencv-python"])
    
    # CUDA 지원 PyTorch 설치
    from scripts.install_packages import install_torch_cuda
    cuda_available, device = install_torch_cuda()
    
    # GPU 정보 확인
    from scripts.install_packages import check_gpu
    gpu_info = check_gpu()
"""
...
def install_packages_with_progress(packages=None, start_time=None):
    ...
    try:
        # 1. NumPy 다운그레이드 (필수 선행 작업)
        # 사용자의 numpy가 어떤 버전이든 다운그레이드. 이미지 1.24.3이 설치 돼 있으면 설치 돼 있다고 하고 넘어감
        ...
        progress_per_package = 70 / total_packages  # 20% ~ 90% 범위 사용
...
def _install_single_package(package, start_time, progress_base=0):
    ...
    Args:
        package (str): 설치할 패키지 이름
        start_time (float): 시작 시간
        progress_base (float): 진행률 기준점
    ...
    try:
        ...
        if process.returncode == 0:
            # 패키지 버전 확인
            version = _get_package_version(package.split('==')[0])  # 버전 지정이 있는 경우 제거
